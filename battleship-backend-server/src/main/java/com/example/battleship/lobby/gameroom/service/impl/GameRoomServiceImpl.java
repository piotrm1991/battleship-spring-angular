...
import static com.example.battleship.lobby.util.SystemMessageFactory.generateSystemMessagePlayerConnected;
import static com.example.battleship.lobby.util.SystemMessageFactory.generateSystemMessagePlayerDisconnected;
import static com.example.battleship.lobby.util.SystemMessageFactory.prepareLobbyMessageRecord;
...
public class GameRoomServiceImpl implements GameRoomService {
  ...
  public Page<GameRoomResponse> getGameRoomsWithOnePlayer(Pageable pageable) {
    ...
    return gameRoomRepository.findByPlayerTwo(pageable,null).map(gameRoomMapper::mapEntityToResponse);
  }
  ...
  public void handleConnectPlayerToGameRoom(Player player) throws IOException {
    ...
    player = playerService.setPlayerStatusOnline(player);
  ...
  private Optional<GameRoom> getGameRoomByPlayer(Player player) {
    ...
    return (gameRoomRepository.findByPlayerOneId(player.getId()).isPresent())
            ?
  ...
  public void handleDisconnectPlayerFromGameRoom(Player player) throws IOException {
    ...
    player = playerService.setPlayerStatusOffline(player);
    Optional<GameRoom> gameRoom = getGameRoomByPlayer(player);
    if (gameRoom.isPresent()) {
  ...
  private void handleDisconnectPlayerPrivateChat(Player player, GameRoom gameRoom) throws IOException {
  ...
  @Override
  public void handleChatMessageFromPlayer(UserMessage userMessage, Player playerFrom) throws IOException {
    log.info("Handling chat message from player: {}", playerFrom.getName());
    Optional<GameRoom> gameRoom = getGameRoomByPlayer(playerFrom);
    if (gameRoom.isPresent()) {
      LobbyMessage lobbyMessage = prepareLobbyMessageRecord(LobbyMessageType.CHAT, playerFrom.getName(), userMessage.content());
      sendMessage(playerFrom, lobbyMessage);
      if (getOtherPlayerForMessage(playerFrom, gameRoom.get()) != null) {
  ...
  private void deleteGameRoom(Player player, GameRoom gameRoom) throws IOException {
    log.info("Handling player one: {}, leaving and deleting game room with id: {}.", player.getName(), gameRoom.getId());
    ...
    if (!Objects.isNull(gameRoom.getPlayerTwo())) {
  ...
  private void disconnectPlayerTwoFromGameRoom(Player player, GameRoom gameRoom) throws IOException {
    log.info("Handling player two: {} leaving the game room with id: {}.", player.getName(), gameRoom.getId());
    handleDisconnectPlayerPrivateChat(player, gameRoom);
    handleDisconnectPlayerGameBoard(player, gameRoom);
  ...
  private Player getOtherPlayerForMessage(Player player, GameRoom gameRoom) {
    ...
    return (gameRoom.getPlayerOne().equals(player)) ? gameRoom.getPlayerTwo() : gameRoom.getPlayerOne();
  }

  @Override
  public boolean checkIfPlayerHasGameRoom(Player player) {
    log.info("Checking if game room of the player: {}, exists.", player.getName());

    return gameRoomRepository.existsByPlayerOneId(player.getId()) || gameRoomRepository.existsByPlayerTwoId(player.getId());
  }
}
